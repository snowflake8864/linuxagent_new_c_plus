...
static const u_char *ktq_md5_body(ktq_md5_t *ctx, const u_char *data,
				  size_t size);
...
void ktq_md5_update(ktq_md5_t *ctx, const void *data, size_t size)
{
	...
	if (used) {
		...
		size -= free;
		(void)ktq_md5_body(ctx, ctx->buffer, 64);
	}

	if (size >= 64) {
		data = ktq_md5_body(ctx, data, size & ~(size_t)0x3f);
		size &= 0x3f;
	}
...
void ktq_md5_data(u_char *md5, const void *data, u_int nlen)
{
	ktq_md5_t c;

	ktq_md5_init(&c);
	ktq_md5_update(&c, data, nlen);
	ktq_md5_final(md5, &c);
}
