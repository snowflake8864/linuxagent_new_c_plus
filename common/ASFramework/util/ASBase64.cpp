...
std::string ASBase64Util::Base64Encode( const std::string& input )
{
	std::string encoded;
	char c;
...
std::string ASBase64Util::Base64Encode(const unsigned char* input,unsigned int nInputlen)
{
	if (!input || nInputlen == 0)
		return "";

	std::string encoded;
	char c;
	const std::string::size_type length = nInputlen;
...
std::string ASBase64Util::Base64Decode( const std::string& input )
{
	char c, d;
	const std::string::size_type length = input.length();
	std::string decoded;
...
unsigned char* ASBase64Util::Base64Decode(const std::string& input,unsigned int& nContLen)
{
	char c, d;
	const std::string::size_type length = input.length();
	unsigned char* decoded = new unsigned char[length * 2];
	if (!decoded)	return NULL;

	unsigned int nDecodeIndex = 0;
	for (std::string::size_type i = 0; i < length; ++i)
	{
		c = table64(input[i]);
		++i;
